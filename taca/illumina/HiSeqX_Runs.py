...
from flowcell_parser.classes import SampleSheetParser, DemuxSummaryParser
...
class HiSeqX_Run(Run):
    ...
    def check_QC(self):
        ...
        status = True # Init status as passed
        ...
        for lane in lanes_to_qc:
            ...
            if self.check_undetermined_reads(lane, max_percentage_undetermined_indexes):
                if self.check_maximum_undertemined_freq(lane, max_frequency_most_represented_und):
                    ...
                    logger.warn("lane {} did not pass the check for most represented undet index. "
                                "Most occurint undet index occurs too ofetn.".format(lane))
                    lane_status= lane_status and False
            else:
                logger.warn("lane {} did not pass the undetermiend qc checks. "
                            "Fraction of undetermined too large.".format(lane))
                lane_status= lane_status and False
            if lane_status:
    ...
    def check_undetermined_reads(self, lane, freq_tresh):
        ...
        if len(undetermined_lane_stats) > 1:
            logger.error("Something wrong in check_undetermined_reads, "
                         "found more than one undetermined sample in one lane")
            return False
    ...
    def check_maximum_undertemined_freq(self, lane, freq_tresh):
        ...
        if freq_most_occuring_undet_index > freq_tresh:
            logger.warn("The most frequent barcode of lane {} ({}) represents {}%, "
                        ...
                        .format(lane, most_frequent_undet_index, freq_most_occuring_undet_index , freq_tresh))
            return False
        else:
            return True
        ...
        for file in glob.glob(os.path.join(run, dmux_folder, "Undetermined*L0?{}*".format(lane))):
            ...
            for index, comp in enumerate(old_name_comps):
                if comp.startswith('L00'):
                    ...
                    old_name_comps[index]=comp.replace('L00','L01') 
            new_name = "_".join(old_name_comps)
            logger.info("Renaming {} to {}".format(file, os.path.join(os.path.dirname(file), new_name)))
            os.rename(file, os.path.join(os.path.dirname(file), new_name))

def _generate_clean_samplesheet(ssparser, fields_to_remove=None, 
                                rename_samples=True, rename_qPCR_suffix = False, fields_qPCR= None):
    """
