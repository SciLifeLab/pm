...
def _run_type(run):
    ...
    :type run: string
    :rtype: String
    :returns: returns a string with the sequencer type name, 
    ...
    try:
        ...
        logger.warn("Cannot find the runParameters.xml file at {}. " + 
        ...
        # Jose : return the run object here instead
...
def transfer_run(run_dir, analysis):
    """
    Interface for click to transfer a run to Uppmax
    :param: string run_dir: the run to transfer
    :param bool analysis: True to trigger analysis
    """
    runObj = _run_type(run_dir)
    if runObj:
        # Jose : runObj can be None
        runObj.transfer_run("nosync", 
                            ...
                            analysis) # do not start the analysis automatically if I force the transfer
...
def run_preprocessing(run, force_trasfer=True, statusdb=True):
    ...
    def _process(run, force_trasfer):
        ...
        if run.is_transferred(t_file):
            # In this case I am either processing a run that is in transfer or that has been already transferred. Do nothing.
        ...
        if run.get_run_status() == 'SEQUENCING':
            ...
            if run.get_run_type() == 'NON-NGI-RUN':
                ...
                logger.warn('Run {} marked as {}, ' + 
                            'TACA will skip this and move the run to no-sync directory'.format(run.id, run.get_run_type()))
    ...
    if run:
        ...
        for data_dir in data_dirs:
            # Jose : double check that this is valid for NextSeq also
